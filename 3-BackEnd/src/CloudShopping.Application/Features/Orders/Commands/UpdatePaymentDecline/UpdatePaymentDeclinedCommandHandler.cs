...
namespace CloudShopping.Application.Features.Orders.Commands.UpdatePaymentDeclined
{
    public sealed class UpdatePaymentDeclinedCommandHandler : IRequestHandler<UpdatePaymentDeclinedCommand, Result>
    {
        ...
        private readonly ILogger<UpdatePaymentDeclinedCommandHandler> _logger;

        public UpdatePaymentDeclinedCommandHandler(
            IOrderRepository orderRepository,
            ITenantProvider tenantProvider,
            IUnitOfWork unitOfWork,
            ILogger<UpdatePaymentDeclinedCommandHandler> logger)
        {
        ...
        public async Task<Result> Handle(UpdatePaymentDeclinedCommand request, CancellationToken cancellationToken)
        {
            ...
            if (order is null)
            {
                _logger.LogWarning("Tentativa de recusar pagamento falhou. Pedido {OrderId} não encontrado.", request.OrderId);
            ...
            {
                _logger.LogWarning(ex, "Erro de validação ao recusar pagamento {PaymentId} do pedido {OrderId}.", request.PaymentId, request.OrderId);
                return Result.Failure(new Error("Order.InvalidPayment", ex.Message));
            }
            _orderRepository.Update(order);
            await _unitOfWork.CommitAsync(cancellationToken);
            _logger.LogInformation("Pagamento {PaymentId} do pedido {OrderId} marcado como recusado com sucesso (Tenant: {TenantId}).",
