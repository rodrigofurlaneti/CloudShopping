...
{
    private readonly Mock<IProcessingLogWriter> writer = new();
    ...
    public async Task Execute(int customer,int product,string operation,int quantity)
    {
        ...
        var services=new ServiceCollection();services.AddLogging();services.AddApplication();services.AddSingleton(commerce.Object);services.AddSingleton(writer.Object);
    ...
    public void Verify(bool accepted,int calls){result.IsSuccess.Should().Be(accepted);writer.Verify(x=>x.WriteAsync(It.Is<LogTracker>(l=>l.IsSuccess==accepted),It.IsAny<CancellationToken>()),Times.Once);commerce.Verify(x=>x.ChangeCart(It.IsAny<int>(),It.IsAny<int>(),It.IsAny<int>(),It.IsAny<string>(),It.IsAny<CancellationToken>()),Times.Exactly(calls));}
}
