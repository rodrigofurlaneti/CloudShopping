...
export function BackofficeLayout({ children }: { children: ReactNode }) {
    ...
    return <div className="admin-shell">
        ...
        <aside id="admin-navigation" className={'admin-sidebar '+(menuOpen?'is-open':'')} onKeyDown={e=>{if(e.key==='Escape')setMenuOpen(false);}}>
            ...
            <nav aria-label="Administração">{NAV_ITEMS.map(item=><Link key={item.to} to={item.to} aria-current={location.pathname===item.to?'page':undefined} onClick={()=>setMenuOpen(false)}><item.icon size={18}/>{item.label}</Link>)}</nav>
