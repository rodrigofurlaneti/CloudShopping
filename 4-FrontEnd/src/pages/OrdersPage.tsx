...
export function OrdersPage() {
    ...
    return <StoreLayout><h1>{id?'Pedido #'+id:'Meus pedidos'}</h1>{(error||result.error)&&<p role="alert" className="notice error">{error||result.error} <Link to="/account">Entrar na minha conta</Link></p>}
        {loading?<p role="status">Carregando pedidos…</p>:id&&detail?<section className="form-card">
            <p className="notice">{detail.orderStatusId===16?'Pedido cancelado.':detail.orderStatusId===1?'Pedido criado — aguardando pagamento.':'Status do pedido: '+detail.orderStatusId}</p>
            {detail.orderStatusId===1&&<p>O pagamento online estará disponível na próxima etapa. A reserva é temporária e pode expirar; não houve cobrança nesta confirmação.</p>}
            ...
            {detail.orderStatusId===1&&detail.reservationState==='Reserved'&&<button disabled={busy} onClick={async()=>{setBusy(true);try{await post('/v1/store/orders/'+id+'/cancel');setVersion(v=>v+1);}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}>Cancelar pedido e liberar reserva</button>}
